<template>
  <div id="flowMainCont" class="flow-main-cont">
    ...
    <div class="draw-wrap">
      <div id="draw" class="main-bg" :style="{transform:`scale(${drawStyle.zoomRate})`,transformOrigin:`${drawStyle.origin}`}"
        @drop.prevent="dropHandle" @dragover.stop.prevent @mousedown="drawLineStart" @mousemove="drawingLine" @mouseup="drawLineEnd"
        @mousewheel.alt.prevent="wheelHandle" @click.stop.prevent="editEnd">
        <div id="drawNode" style="position: absolute;">
          <flow-node v-for="(item,index) in nodeData" :key="index" :option="item"></flow-node>
        </div>
        <tool-menu :ulStyle="'width:100px;text-align:center'" :visible.sync="visible" :menuData="menuData" @selItme="deleteHandle"
          :style="{top:menuInfo.top,left:menuInfo.left}"></tool-menu>
      </div>
    </div>
  </div>
</template>
        ...
        for (var key in this.lineData) {
          if (this.lineData[key].startNode === id) {
            ...
            let {
              left,
              top
            } = this.computeLine(direction, this.selNodeInfo);
